...
class controller:
    ...
    def control_it(self):
        for node in reversed(self.path):
            ...
            spot_px = node.tf_gazebo_to_px(self.grid.meter_per_pixel)
            # self.img_matrix[spot_px[0],spot_px[1]] = 200/255.
            self.img_matrix[spot_px[1]: self.grid.meter_per_pixel + spot_px[1]
                                                  , spot_px[0]: self.grid.meter_per_pixel + spot_px[0]] = 210/255.


            cv2.imshow("gazebo" , self.img_matrix.astype("float"))

            if cv2.waitKey(1) == ord("q"):
                cv2.destroyAllWindows()

            if abs(goal[0]-self.x) >= 0.1:
                # print("A11")
                # print("theta" , self.theta)
                # print("tan" ,math.atan((goal[1] - self.y) / (goal[0] - self.x)))
                # try:
                #     if abs(goal[0]-self.x) >= 0.1:
                #         angle = math.atan((goal[1] - self.y) / (goal[0] - self.x))
                # except:
                ...
                while abs(self.theta - angle) >= 0.01:
                    # print("B11")
                    #print("X-theta" , self.theta ,"X-angle" , angle)
                    # print("tan" ,math.atan((goal[1] - self.y) / (goal[0] - self.x)))
                    self.vel_cmd.angular.z = (self.theta - angle) * -0.5
                    #print("D11")
            ...
            if abs(goal[1]-self.y) >= 0.1:
                ...
                while abs(self.theta - angle) >= 0.01:
                    # print("B11")
                    #print("Y-theta" , self.theta ,"Y-angle" , angle)
                    # print("tan" ,math.atan((goal[1] - self.y) / (goal[0] - self.x)))
                    self.vel_cmd.angular.z = (self.theta - angle) * -0.5
                    #print("D11")
